...
function App() {
  ...
  useEffect(() => {
    axios.get('http://127.0.0.1:8001/data')
      .then(response => {
  ...
  return (
    <div className="bg-gray-100 min-h-screen p-5">
      <div className="container mx-auto px-4 py-8">
        ...
        <MapContainer center={[45.0, 7.6]} zoom={13} className="rounded shadow h-64" id="map">
          <TileLayer
            attribution='© OpenStreetMap contributors'
            url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png"
          />
          {markers.map(marker => (
            <Marker key={marker.ID_ELEMENTO} position={[marker.LATITUDINE, marker.LONGITUDINE]} icon={markerIcons[marker.STATO]}>
              <Popup>
